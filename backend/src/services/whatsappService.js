    ...
    try {
      ...
      if (this.useTemplates && this.templateNames.appointmentReminder) {
        try {
          ...
          const templateParams = this.prepareAppointmentReminderTemplateParams(appointmentData);
          console.log('[WhatsApp Service] Reminder template parameters:', {
            count: templateParams.length,
            params: templateParams
          });
          const templateResult = await this.sendTemplateMessage(
  ...
  prepareAppointmentReminderTemplateParams(appointmentData) {
    ...
    if (appointmentDate) {
      try {
        ...
        console.error('[WhatsApp Template] Error formatting reminder date:', error);
      }
    }
    ...
    if (!appointmentTime && appointmentDate) {
      try {
        ...
        console.error('[WhatsApp Template] Error formatting reminder time:', error);
      }
    } else if (appointmentTime) {
      // Convert "10:00 am" to "10 am" or "10:00am" to "10 am", keep "10:30 am" as is
      displayTime = appointmentTime.replace(/:00(\s|$)/g, '$1').toLowerCase().trim();
