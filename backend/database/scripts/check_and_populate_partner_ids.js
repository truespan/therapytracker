/**
 * Script to check and populate Partner IDs for existing partners
 * This script will:
 * 1. Check if partner_id column exists
 * 2. Add the column if it doesn't exist
 * 3. Populate Partner IDs for all partners without IDs
 * 
 * Usage: node backend/database/scripts/check_and_populate_partner_ids.js
 */
  ...
  try {
    ...
    if (errorCount === 0 && successCount > 0) {
      ...
      try {
        ...
        console.log('⚠ Note: Constraints may already exist or need to be added manually');
      }
      
      // Display all Partner IDs
      console.log('\n=== All Partner IDs ===');
      const finalResult = await db.query('SELECT id, name, partner_id FROM partners ORDER BY id');
